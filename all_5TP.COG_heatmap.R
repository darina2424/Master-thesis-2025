...
heatmap_data <- read.xlsx("HHeatmap_data_all_TP5.xlsx")
...
pdf("all_5TP.COG_bar.pdf", width = 14, height = 2)
...
pdf("all_5TP.COG_heatmap.top25.pdf", width = 10, height = 12)
print(top_heatmap)
dev.off()
...
pdf("all_5TP.COG_heatmap.pdf", width = 10, height = 8)

for (i in seq_along(heatmap_list)) {
  p <- heatmap_list[[i]] + 
    theme(axis.text.y = element_text(size = y_text_size, lineheight = 1))
  grid.arrange(p)
}
