...
#data <- read.csv("filtered_output.csv")

colnames(data) <- make.names(colnames(data))

result <- data %>%
  group_by(time.point.7, ProteinID) %>%
  summarise(
...
periods <- c("Preparation", "Training", "Rest", "Climbing", "Back")

filtered_result <- result %>%
  filter(time.point.7 %in% periods)
...
protein_time_combinations <- expand.grid(
  ...
  time.point.7=periods
)

filtered_result <- protein_time_combinations %>% 
  left_join(., filtered_result, by=c("ProteinID", "time.point.7"))

dcast_filtered_result <- reshape2::dcast(filtered_result %>% select(ProteinID,time.point.7,Mean_relab),
                                         ProteinID~time.point.7, value.var="Mean_relab") %>% 
  select(c("ProteinID", all_of(periods)))

dcast_filtered_result <- dcast_filtered_result %>% 
  rowwise() %>% 
  mutate(
    Mean = mean(c(Preparation, Training, Rest, Climbing, Back), na.rm = TRUE),
    StandardDeviation = sd(c(Preparation, Training, Rest, Climbing, Back), na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(
    Z_Preparation = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (Preparation - Mean) / StandardDeviation, 0),
    Z_Training = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (Training - Mean) / StandardDeviation, 0),
    Z_Rest = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (Rest - Mean) / StandardDeviation, 0),
    Z_Climbing = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (Climbing - Mean) / StandardDeviation, 0),
    Z_Back = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (Back - Mean) / StandardDeviation, 0)
  ) %>% unique()

Zscore_filtered_result <- dcast_filtered_result[,c(1,grep("Z_", colnames(dcast_filtered_result)))]
save(Zscore_filtered_result, file="ZAMG_5TP.RData")

correlation_results <- reshape2::melt(Zscore_filtered_result, id = "ProteinID") %>%
  mutate(time_index=case_when(variable=="Z_Preparation" ~ 1,
                              variable=="Z_Training" ~ 2,
                              variable=="Z_Rest" ~ 3,
                              variable=="Z_Climbing" ~ 4,
                              variable=="Z_Back" ~ 5,
                              TRUE ~ NA)) %>% 
  group_by(ProteinID) %>%
  summarise(
...
top_100_long <- top_100_extended %>%
  ...
  mutate(Timepoint = factor(Timepoint, levels = paste0("Z_", periods),
                            labels = paste0(periods)))

min_z <- min(top_100_long$Z_score, na.rm = TRUE)
max_z <- max(top_100_long$Z_score, na.rm = TRUE)

# pdf("B_AMG_5TP.pdf",width = 17,height = 14)
# ggplot() +
...
#   ) +
#   labs(title = "Correlation of Protein Relative Abundance across 5 Timepoints (AMG)")
# dev.off()
