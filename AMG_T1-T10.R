...
periods <- c("T1", "T2", "T3", "T4", "T5", "T6", "T7", "T8", "T9", "T10")
...
dcast_filtered_result <- dcast_filtered_result %>% 
  rowwise() %>% 
  mutate(
    Mean = mean(c(T1, T2, T3, T4, T5, T6, T7, T8, T9, T10), na.rm = TRUE),
    StandardDeviation = sd(c(T1, T2, T3, T4, T5, T6, T7, T8, T9, T10), na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(
    Z_T1 = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (T1 - Mean) / StandardDeviation, 0),
    Z_T2 = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (T2 - Mean) / StandardDeviation, 0),
    Z_T3 = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (T3 - Mean) / StandardDeviation, 0),
    Z_T4 = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (T4 - Mean) / StandardDeviation, 0),
    Z_T5 = ifelse(!is.na(StandardDeviation) & StandardDeviation != 0, (T5 - Mean) / StandardDeviation, 0),
...
save(Zscore_filtered_result, file="ZAMG_T1-T10.RData")

correlation_results <- reshape2::melt(Zscore_filtered_result, id = "ProteinID") %>%
  mutate(time_index=as.numeric(gsub(pattern="Z_T",replacement="", variable))) %>% 
  group_by(ProteinID) %>%
  summarise(
...
top_100_long <- top_100_extended %>%
  ...
  mutate(Timepoint = factor(Timepoint, levels = paste0("Z_T", 1:10),
                            labels = paste0("T", 1:10)))

min_z <- min(top_100_long$Z_score, na.rm = TRUE)
max_z <- max(top_100_long$Z_score, na.rm = TRUE)

# pdf("B_AMG_T1_T10.pdf",width = 17,height = 14)
# ggplot() +
...
#   ) +
#   labs(title = "Correlation of Protein Relative Abundance across T1-T10 Timepoints (AMG)")
# dev.off()
