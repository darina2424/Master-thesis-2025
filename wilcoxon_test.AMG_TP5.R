...
filtered_data$time.point.7 <- factor(filtered_data$time.point.7, levels = periods)

data_filtered <- data %>%
  inner_join(filtered_data, by = c("ProteinID", "time.point.7")) %>% 
  arrange(ProteinID) 
...
data_filtered <- data_filtered %>%
  ...
  mutate(annotation = str_wrap(sapply(AMG_KO_name, trim_annotation), width = 140))
...
wilcox_results <- data_filtered %>%
  group_by(ProteinID) %>%
  filter(n_distinct(time.point.7) > 1) %>%  
  summarise(p_values = list({
    time_points <- unique(time.point.7)
    ...
    result_list <- combn(time_points, 2, function(pair) {
      ...
      data_t1 <- Relative.abundance[time.point.7 == t1]
      data_t2 <- Relative.abundance[time.point.7 == t2]
...
heatmap_data <- filtered_data %>%
  ...
  dcast(ProteinID ~ time.point.7, value.var = "Mean_relab")
...
# pdf("P_valueAMG.TP5.pdf",width = 20,height = 24)
# gridExtra::grid.arrange(p$gtable)
# dev.off()
# 
# openxlsx::write.xlsx(wilcox_results, file = "AMG_TP5_pvalue.xlsx")
